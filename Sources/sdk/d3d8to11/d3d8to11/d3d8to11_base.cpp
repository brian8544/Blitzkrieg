...
void Direct3D8::create_native()
{
	...
	for (UINT adapter_index = 0; adapter_index < MAX_ADAPTERS && result == S_OK; adapter_index++)
	{
		...
		// D3D8 adapter indices must be contiguous.
		const UINT d3d_adapter_index = m_current_adapter_count;
		if (d3d_adapter_index >= MAX_ADAPTERS)
		{
			break;
		}

		m_current_adapter_descs[d3d_adapter_index] = adapter_desc;
		...
		for (const D3DFORMAT format : ADAPTER_FORMATS)
		{
			...
			auto& stored_modes = m_current_adapter_modes[d3d_adapter_index];
			stored_modes.insert(stored_modes.end(), modes.begin(), modes.end());
			m_current_adapter_mode_count[d3d_adapter_index] += static_cast<UINT>(modes.size());
		}
...
HRESULT STDMETHODCALLTYPE Direct3D8::GetAdapterIdentifier(UINT Adapter, DWORD Flags, D3DADAPTER_IDENTIFIER8* pIdentifier)
{
	...
	if (pIdentifier == nullptr || Adapter >= m_current_adapter_count)
	{
	...
	*pIdentifier = {};
	...
	const DXGI_ADAPTER_DESC& desc = m_current_adapter_descs[Adapter];
	if (::WideCharToMultiByte(CP_ACP, 0, desc.Description, -1,
	                          pIdentifier->Description, static_cast<int>(sizeof(pIdentifier->Description)),
	                          nullptr, nullptr) == 0)
	{
		sprintf_s(pIdentifier->Description, sizeof(pIdentifier->Description), "D3D11 adapter %u", Adapter);
	}

	strcpy_s(pIdentifier->Driver, sizeof(pIdentifier->Driver), "d3d8to11");
...
{
	if (!pCaps || Adapter >= m_current_adapter_count)
	{
		return D3DERR_INVALIDCALL;
	}
