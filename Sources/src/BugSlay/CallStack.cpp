...
{
	char buff[1024];
	sprintf_s( buff, "(0x%X) %s", dxrval, DXErrorToString(dxrval) );
...
LONG STDCALL CrashHandlerFilter( EXCEPTION_POINTERS *pExPtrs )
{
  ...
  CCallStackEntryList entries;
	...
	char buff[128];
	if ( !entries.empty() )
		sprintf_s( buff, "First-chance exception in %s: 0x%.8X: %s.", entries.front().szModuleName.c_str(), pExPtrs->ExceptionRecord->ExceptionCode, ConvertSimpleException(pExPtrs->ExceptionRecord->ExceptionCode) );
	else
		sprintf_s( buff, "Unhandled Exception (0x%X) at %p", pExPtrs->ExceptionRecord->ExceptionCode, pExPtrs->ExceptionRecord->ExceptionAddress );
	// trace debug info
	OutputDebugString( "*********************************************************************************************************\n" );
	OutputDebugString( buff );
	OutputDebugString( "\n" );
