																						...
																						LPUINT          puiHooked )
{
  ...
  while ( NULL != pOrigThunk->u1.Function )
  {
    ...
    if ( IMAGE_ORDINAL_FLAG != (pOrigThunk->u1.Ordinal & IMAGE_ORDINAL_FLAG) )
    {
      ...
      for ( i=0; i<uiCount; i++ )
      {
        if ( ( paHookArray[i].szFunc[0] == pByName->Name[0] ) && ( 0 == _stricmp(paHookArray[i].szFunc, (char*)pByName->Name) ) )
        {
      ...
      if ( TRUE == bDoHook )
      {
        ...
        // Do the actual hook.
        pRealThunk->u1.Function = static_cast<DWORD>( reinterpret_cast<DWORD_PTR>(paHookArray[i].pProc) );
