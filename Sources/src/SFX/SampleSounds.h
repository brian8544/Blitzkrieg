...
class CSoundSample : public ISharedResource
{
	...
	void Close();
public:
	CSoundSample() : sample( nullptr ), nMode( FMOD_2D ), bLooped( false ), fMinDistance( 45.0f ) {}
	~CSoundSample() { Close(); }

	void SetSample( FMOD::Sound *_sample );
	FMOD_MODE GetMode() const { return nMode; }
	bool IsLooped() const { return bLooped; }
	FMOD::Sound* GetInternalContainer() { Load(); return sample; }
...
public:
	...
	void STDCALL SetVolume( float ) {}
	float STDCALL GetVolume() const { return 1.0f; }
	void STDCALL SetPan( float ) {}
...
public:
	...
	int STDCALL Play();
	void STDCALL SetPosition( const CVec3 & ) {}
...
class CSound3D : public CBaseSound
{
	...
	CVec3 vLastPos;
public:
	CSound3D() : vPos( VNULL3 ), bDopplerFlag( false ), lastUpdateTime( 0 ), vLastPos( VNULL3 ) {}
	virtual ~CSound3D() {}
	int STDCALL Visit( interface ISFXVisitor *pVisitor );
	int STDCALL Play();
