...
public:
	...
	void ReadCString( std::string &res ) { int nLeng = static_cast<int>( strlen( (char*)pCurrent ) ); res.assign( (char*)pCurrent, nLeng ); pCurrent += nLeng + 1; CheckCurrentR(); }
	void WriteCString( const char *pSrc ) { int nLeng = static_cast<int>( strlen( pSrc ) ); memcpy( pCurrent, pSrc, nLeng + 1 ); pCurrent += nLeng + 1; CheckCurrentW(); }
